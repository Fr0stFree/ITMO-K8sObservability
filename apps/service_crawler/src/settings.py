...
class CrawlerServiceSettings(BaseSettings):
    concurrent_workers: int = Field(default=1, ge=1, le=100, alias="CRAWLER_CONCURRENT_WORKERS")
    worker_request_timeout: dt.timedelta = Field(
        default=dt.timedelta(seconds=10), alias="CRAWLER_WORKER_REQUEST_TIMEOUT"
    )
